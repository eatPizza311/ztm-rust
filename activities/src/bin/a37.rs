...
#[derive(Debug, Error)]
enum RgbError {
    #[error("falied to parse hex digit: {0}")]
    ParseError(std::num::ParseIntError),
    #[error("Missing hash symbol")]
    MissingHashError,
    #[error("The hex code length is not 7")]
    LengthError,
}
...
impl TryFrom<&str> for Rgb {
    ...
    fn try_from(hex: &str) -> Result<Self, Self::Error> {
        if !hex.starts_with('#') {
            return Err(RgbError::MissingHashError);
        }
        if hex.len() != 7 {
...
// This can be replace by thiserror's #[from] macro
impl From<std::num::ParseIntError> for RgbError {
    fn from(err: std::num::ParseIntError) -> RgbError {
...
mod test {
    ...
    fn fails_when_missing_hash() {
        assert_eq!(
            ...
            true,
            "should be an error when missing has symbolh"
        );
    }
